...
export default function EvaluationPlugin(editor) {
  ...
  if (state.elections.length > 0) {
    tab.addRevealSection(
      ...
      {
        isOpen:
          state.population.subgroups.length <= 1 && state.vap === undefined
            ? true
            : false,
      }
    );
  ...
  if (
    ...
    tab.addRevealSection(
      "Contiguity",
      (uiState, dispatch) =>
        ContiguitySection(state.contiguity, uiState, dispatch),
      {
        isOpen: true,
      }
    );
    tab.addRevealSection(
      "Cut edges",
      (uiState, dispatch) => CutEdgesSection(state, uiState, dispatch),
      {
        isOpen: true,
      }
    );
